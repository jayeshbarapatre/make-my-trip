...
import { vendorTrainsService } from '../services/vendorService'
...
const VendorTrains = () => {
  ...
  const fetchTraines = async () => {
    try {
      setLoading(true)
      const response = await vendorTrainsService.getAll()
  ...
  const handleSubmitForApproval = (id) => {
    setConfirmDialog({
      ...
      message: 'This will send your train for admin review. Are you sure you want to submit it?'
    })
  }
  ...
  const handleDelete = (id) => {
    setConfirmDialog({
      ...
      trainId: id,
      title: 'Delete Train',
      message: 'Are you sure you want to delete this train? This action is permanent and cannot be undone.'
    })
  }
  ...
  const confirmAction = async () => {
    ...
    try {
      setProcessingId(confirmDialog.trainId)
      if (confirmDialog.type === 'submit') {
        await vendorTrainsService.submit(confirmDialog.trainId)
        toast.success('Train submitted for approval')
      } else if (confirmDialog.type === 'delete') {
        await vendorTrainsService.delete(confirmDialog.trainId)
        toast.success('Train deleted successfully')
        setTraines(trains.filter(b => b.id !== confirmDialog.trainId))
      }
      fetchTraines()
    ...
    return (
      <VendorLayout>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '100px 0' }}>
          ...
          <p style={{ color: 'var(--text-secondary)' }}>Loading your trains...</p>
        </div>
      </VendorLayout>
    )
  }
        ...
        <div className="card" style={{ marginBottom: '24px' }}>
          <div className="card-body" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div style={{ flex: 1, minWidth: '280px' }}>
              <h1 style={{ fontSize: '24px', fontWeight: 600, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.02em' }}>My Traines</h1>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px 24px', marginTop: '12px', fontSize: '13px', color: 'var(--text-secondary)' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <i className="fas fa-train" style={{ color: 'var(--text-muted)' }}></i> Total: <strong style={{ color: 'var(--text-primary)' }}>{trains.length}</strong>
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <i className="fas fa-pencil-alt" style={{ color: 'var(--text-muted)' }}></i> Draft: <strong style={{ color: 'var(--text-primary)' }}>{trains.filter(b => b.listingStatus === 'DRAFT').length}</strong>
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <i className="fas fa-hourglass-half" style={{ color: '#d39e00' }}></i> Pending: <strong style={{ color: 'var(--text-primary)' }}>{trains.filter(b => b.listingStatus === 'PENDING_APPROVAL').length}</strong>
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <i className="fas fa-check-circle" style={{ color: '#2ec158' }}></i> Approved: <strong style={{ color: 'var(--text-primary)' }}>{trains.filter(b => b.listingStatus === 'APPROVED').length}</strong>
                </span>
              </div>
            </div>
            <button className="btn btn-primary" onClick={() => { setEditingTrainId(null); setShowFormModal(true); }} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: 'auto' }}>
              <i className="fas fa-plus"></i> Add New Train
            </button>
          </div>
        </div>
        ...
        {trains.length === 0 ? (
          <div className="card" style={{ padding: '60px 20px', textAlign: 'center' }}>
            <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'var(--surface2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', border: '1px solid var(--border)' }}>
              <i className="fas fa-train" style={{ fontSize: '24px', color: 'var(--text-muted)' }}></i>
            </div>
            <h3 style={{ fontSize: '18px', fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 8px' }}>No trains found</h3>
            <p style={{ color: 'var(--text-secondary)', margin: '0 0 24px', fontSize: '14px' }}>You haven't listed any trains yet. Get started by adding your first route.</p>
            <button className="btn btn-primary" onClick={() => { setEditingTrainId(null); setShowFormModal(true); }} style={{ width: 'fit-content', margin: '0 auto' }}>
              <i className="fas fa-plus" style={{ marginRight: '8px' }}></i> Add New Train
            </button>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '20px' }}>
            {trains.map(train => (
              <div key={train.id} className="card" style={{ display: 'flex', flexDirection: 'column' }}>
                <div className="card-body" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px', padding: '24px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <h3 style={{ fontSize: '18px', fontWeight: 600, margin: '0 0 4px', color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {train.operatorName} - {train.trainNumber}
                      </h3>
                      <p style={{ fontSize: '13.5px', color: 'var(--text-secondary)', margin: 0, display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <i className="fas fa-route"></i> {train.departure?.city} to {train.arrival?.city}
                      </p>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                      <span className={`badge ${getStatusBadgeClass(train.listingStatus)}`} style={{ textTransform: 'uppercase', fontSize: '10px', letterSpacing: '0.04em', fontWeight: 700, padding: '4px 8px' }}>
                        {train.listingStatus.replace(/_/g, ' ')}
                      </span>
                      {train.type && (
                  ...
                  {/* Rejection Alert */}
                  {train.listingStatus === 'REJECTED' && train.rejectionReason && (
                    <div style={{ padding: '12px', background: 'rgba(255, 77, 79, 0.05)', borderRadius: '10px', border: '1px dashed rgba(255, 77, 79, 0.3)', color: '#ff4d4f', fontSize: '12.5px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                      ...
                      <div>
                        <strong style={{ fontWeight: 600 }}>Rejection Reason:</strong> {train.rejectionReason}
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', background: 'var(--surface2)', padding: '12px', borderRadius: '12px', border: '1px solid var(--border)' }}>
                    <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      ...
                      <p style={{ fontSize: '14px', fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>₹{train.price?.toLocaleString() || 0}</p>
                    </div>
                    <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      ...
                      <p style={{ fontSize: '14px', fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>{train.seatsAvailable}</p>
                    </div>
                    <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      ...
                      <p style={{ fontSize: '14px', fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>{train.departureTime}</p>
                    </div>
                  </div>

                  {/* Actions */}
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: 'auto' }}>
                    <button className="btn btn-sm" onClick={() => { setEditingTrainId(train.id || train._id); setShowFormModal(true); }} style={{ flex: 1, display: 'flex', gap: '6px', justifyContent: 'center', alignItems: 'center' }}>
                      <i className="fas fa-edit" style={{ opacity: 0.7 }}></i> Edit
                    </button>
                  </div>
                  {['DRAFT', 'REJECTED'].includes(train.listingStatus) && (
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button className="btn btn-sm btn-primary" onClick={() => handleSubmitForApproval(train.id || train._id)} style={{ flex: 2, display: 'flex', gap: '6px', justifyContent: 'center', alignItems: 'center' }}>
                        <i className="fas fa-paper-plane" style={{ opacity: 0.9 }}></i> Submit for Approval
                      </button>
                      <button className="btn btn-sm btn-danger" onClick={() => handleDelete(train.id || train._id)} style={{ flex: 1, display: 'flex', gap: '6px', justifyContent: 'center', alignItems: 'center' }}>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: '16px 24px', background: 'var(--surface2)', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              ...
              >
                {processingId === confirmDialog.trainId ? (
                  <span className="loading loading-spinner" style={{ width: '16px', height: '16px' }}></span>
                ) : (
                  confirmDialog.type === 'delete' ? 'Delete Train' : 'Submit Train'
                )}
              </button>
            </div>
          </div>
