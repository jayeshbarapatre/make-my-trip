...
const VendorTrainForm = ({ trainId, onClose, onSuccess }) => {
  const isEditing = !!trainId
  const { theme, accentColor } = useTheme()

  const [formData, setFormData] = useState({
    operatorName: '', trainNumber: '', type: 'AC',
  ...
  const loadTrain = async () => {
    try {
      ...
      const trainsList = response.data.data.traines || response.data.data.trains || []
      const train = trainsList.find(b => b.id === trainId || b._id === trainId)
      if (train) {
        setFormData(train)
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.operatorName || !formData.trainNumber) {
      toast.error('Please fill required train details')
      return
    }
    ...
    try {
      setSubmitting(true)
      if (isEditing) {
        await vendorTrainsService.update(trainId, formData)
        toast.success('Train updated successfully')
      } else {
        await vendorTrainsService.create(formData)
  ...
  return createPortal(
    ...
    >
      <style>{`
        ...
        .train-modal-container input:focus, .train-modal-container select:focus {
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 600, margin: 0, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'rgba(234, 179, 8, 0.15)', color: '#eab308', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}>
              <i className={isEditing ? 'fas fa-pen' : 'fas fa-train'}></i>
            </div>
            {isEditing ? 'Edit Train Details' : 'Add New Train'}
          </h2>
          <button 
            type="button" 
            onClick={onClose} 
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
              ...
              <div style={{ marginBottom: '32px' }}>
                ...
                <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
                  <div style={{ flex: 1 }}>
                    <label style={labelStyle}>Operator/Railway <span style={{ color: '#ff4d4f' }}>*</span></label>
                    <input name="operatorName" type="text" value={formData.operatorName} onChange={handleChange} placeholder="e.g., RedTrain Travels" required style={inputStyle} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={labelStyle}>Train Number <span style={{ color: '#ff4d4f' }}>*</span></label>
                    <input name="trainNumber" type="text" value={formData.trainNumber} onChange={handleChange} placeholder="e.g., RB-001" required style={inputStyle} />
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '16px' }}>
                  <div style={{ flex: 1 }}>
                    <label style={labelStyle}>Train Type <span style={{ color: '#ff4d4f' }}>*</span></label>
              ...
              <button type="submit" className="btn btn-primary" disabled={submitting} style={{ minWidth: '140px' }}>
                {submitting ? (
                  ...
                  <>
                    ...
                    {isEditing ? 'Save Changes' : 'Add Train'}
                  </>
                )}
              </button>
            </div>
...
export default VendorTrainForm
